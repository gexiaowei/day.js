...
public class LunarDateConverter {
    ...
    /// - Parameter date: 公历日期
    /// - Returns: 农历年、月、日的元组
    public static func getLunarComponents(from date: Date) -> (year: Int, month: Int, day: Int) {
    ...
    ///   - repeatCycle: 重复周期
    /// - Returns: 下一个重复日期（公历）
    public static func nextLunarDate(from date: Date, repeatCycle: RepeatCycle) -> Date? {
    ...
    /// - Parameter date: 公历日期
    /// - Returns: 农历日期字符串
    public static func formatLunarDate(from date: Date) -> String {
        let lunarComponents = getLunarComponents(from: date)
